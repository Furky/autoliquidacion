...
<head>
    ...
    <title>Editar Datos de la EELL/Ayuntamiento</title>
    <!-- Enlaces a Bootstrap -->
    ...
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-12"> <!-- Espacio para el mensaje de bienvenida y formulario -->
                ...
                <form action="{{ route('editardatosentidad') }}" method="POST">
                    @csrf
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="direccion">Dirección</label>
                            <input type="text" class="form-control" id="direccion" name="direccion" value="{{ $entidad->direccion }}" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="cp">Código Postal</label>
                            <input type="text" class="form-control" id="cp" name="cp" value="{{ $entidad->cp }}" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="poblacion">Población</label>
                            <input type="text" class="form-control" id="poblacion" name="poblacion" value="{{ $entidad->poblacion }}" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="provincia">Provincia</label>
                            <input type="text" class="form-control" id="provincia" name="provincia" value="{{ $entidad->provincia }}" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="telefono">Teléfono</label>
                            <input type="text" class="form-control" id="telefono" name="telefono" value="{{ $entidad->telefono }}" required>
                        </div>
                    </div>
                    <!-- Botones -->
                ...
                <!-- Botón Volver -->
                <div class="center mt-4">
                    <button type="button" class="btn btn-danger" style="width: 20%" onclick="window.location='{{ route("panelentidad") }}'">Volver</button>
                </div>
            </div>
        </div>
    </div>
